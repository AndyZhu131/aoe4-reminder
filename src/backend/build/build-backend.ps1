...
if (-not (Test-Path $venvPython)) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to create the release build virtual environment."
    }
}

& $venvPython -m pip install --disable-pip-version-check -r (Join-Path $sourceRoot "requirements-build.txt")
if ($LASTEXITCODE -ne 0) {
...
& $venvPython -m PyInstaller `
    ...
    --add-data "$(Join-Path $sourceRoot 'templates');templates" `
    ...
    (Join-Path $sourceRoot "backend\app\aoe4_assistant.py")

if ($LASTEXITCODE -ne 0) {
    throw "PyInstaller failed; the installer was not created from a current backend build."
}

if (-not (Test-Path (Join-Path $distributionRoot "aoe4-assistant\aoe4-assistant.exe"))) {
    throw "PyInstaller completed without producing aoe4-assistant.exe."
}
